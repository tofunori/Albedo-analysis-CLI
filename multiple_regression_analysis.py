...
class MultipleRegressionAnalyzer:
    ...
    def __init__(self, output_base_dir: str = "outputs"):
        ...
        self.output_dir = Path(output_base_dir) / f"multiple_regression_{timestamp}"
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        merra2_path = r"D:\Downloads\Haig_Glacier_Climate_JuneSept_Daily_MERRA2_Speciated_AOD - Haig_Glacier_Climate_JuneSept_Daily_MERRA2_Speciated_AOD.csv"
        merra2_df = pd.read_csv(merra2_path)
        ...
        modis_path = r"D:\Downloads\MODIS_Terra_Aqua_MultiProduct_2002-01-01_to_2025-01-01.csv"
        modis_df = pd.read_csv(modis_path)
        modis_df['date'] = pd.to_datetime(modis_df['date'])
        logger.info(f"Loaded {len(modis_df)} MODIS records")
        
        # Filter MODIS data to June-September 2002-2024 (full melt season coverage)
        logger.info("Filtering MODIS data to June-September 2002-2024...")
        modis_df = modis_df[
            (modis_df['date'].dt.month.isin([6, 7, 8, 9])) &
            (modis_df['date'].dt.year >= 2002) &
            (modis_df['date'].dt.year <= 2024)
        ]
...
def main():
    ...
    analyzer = MultipleRegressionAnalyzer()
