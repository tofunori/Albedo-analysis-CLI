#!/usr/bin/env python3
"""
Haig Glacier Trend Analysis (final)
...
CONFIG = {
    ...
    'output': {
        ...
        'base_dir': 'outputs',
        'trends_plot_filename': 'haig_trends_analysis.png',
        'correlations_plot_filename': 'haig_correlations_analysis.png',
        'summary_template': {
            'analysis_type': 'Sen Slope and Mann-Kendall Trend Analysis',
            'description': 'Non-parametric trend analysis of Haig glacier albedo (2002-2024)'
        }
    },
    'sensitivity': {
        'run': True,
...
class DataLoader:
    ...
    def load_haig_data_complete(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"Loaded {len(modis):,} MODIS, {len(aws):,} AWS albedo, and {len(temp):,} temperature records for Haig glacier")
...
def append_table2_like_summary(output_manager: OutputManager,
                               elev_med: float,
    ...
    lines = []
    lines.append("August trends by altitude band (paper-style)")
    lines.append("--------------------------------------------------")
    lines.append(f"Month = {month}, Snow & Ice threshold (glacier_fraction) ≥ {min_gf:.2f}")
    lines.append(f"Median elevation (approx.) = {('NA' if not np.isfinite(elev_med) else f'{elev_med:.1f} m')}")
    lines.append("")
    ...
    lines.append("")
    lines.append("Snow & Ice (edge-free):")
    for band in ['>Median','Median','<Median']:
        r = pure_res.get(band, {})
        lines.append(f"  {band:<8}  Sen slope = {fmt(r.get('sen'))} /yr  | MK p = {fmt(r.get('mk_p'))} | n_years = {r.get('n', 0)}")
    lines.append("")
    # Write inside the current run directory managed by OutputManager
    try:
        run_dir = Path(getattr(output_manager, 'base_dir', getattr(output_manager, 'output_dir', 'outputs')))
        run_dir.mkdir(parents=True, exist_ok=True)
        summary_path = run_dir / SUMMARY_TXT_NAME
        with open(summary_path, 'a', encoding='utf-8') as f:
            f.write("\n".join(lines) + "\n")
        output_manager.log_file_saved(summary_path, "text (updated Table-2-style)")
        logger.info("Appended Table-2-style altitude-band August trends to summary.txt")
    except Exception as e:
        logger.warning(f"Could not append altitude-band summary: {e}")
...
def append_stats_to_existing_summary(output_manager: OutputManager,
                                     stats_map: Dict[str, Any],
                                     sensitivity_notes: List[str]):
    lines = []
    for key, s in stats_map.items():
        nice = key.replace('_annual', '').replace('modis_', 'MODIS ').replace('_', ' ').title()
        ci_lo, ci_hi = s['slope_confidence_interval']
        lines.append(
            f"{nice}:\n"
            f"  n = {s['n_observations']}, duration = {s['duration_years']:.1f} years\n"
            f"  Kendall tau = {s['kendall_tau']:.3f}, p = {s['p_value']:.3f}"
            f"{' (PW)' if s.get('prewhitened') else ''}\n"
            f"  Sen slope = {s['sen_slope_per_year']:+.4f} per year\n"
            f"  95% CI = [{ci_lo:+.4f}, {ci_hi:+.4f}]\n"
            f"  Trend = {s['mann_kendall_trend']} "
            f"({'significant' if s['statistical_significance'] else 'not significant'})\n"
        )
    stats_block = (
        "\nTrend statistics by series:\n"
        "-----------------------------\n"
        + ("\n".join(lines) if lines else "No trend series available.\n")
        + ("\nSensitivity notes:\n-------------------\n" + "\n".join(sensitivity_notes) if sensitivity_notes else "")
        + "\n"
    )
    summary_path = Path(output_manager.base_dir) / SUMMARY_TXT_NAME
    try:
        with open(summary_path, 'a', encoding='utf-8') as f:
            f.write(stats_block)
        output_manager.log_file_saved(summary_path, "text (updated)")
        logger.info(f"Appended detailed stats to {summary_path}")
    except Exception as e:
        logger.error(f"Could not append to {summary_path}: {e}")

def generate_trend_summary_and_readme(output_manager: OutputManager,
    ...
    try:
        ...
        key_findings = []
        modis_key = next((k for k in stats_map if 'MODIS' in k or 'modis' in k), None)
        if modis_key:
            s = stats_map[modis_key]
            slope = s['sen_slope_per_year']; trend = s['mann_kendall_trend']
            sig = "significant" if s['statistical_significance'] else "non-significant"
            key_findings.append(f"MODIS albedo shows {sig} {trend} trend of {slope:+.4f} units/yr (p={s['p_value']:.3f})"
                                f"{' with prewhitening' if s['prewhitened'] else ''}")
        ...
        output_manager.save_readme(
            ...
            additional_info={
                ...
                'Statistical Significance': f"alpha = {CONFIG['trend_analysis']['alpha']} (95% confidence level)",
                ...
                'Seasonal Definition': f"Melt season months: {CONFIG['trend_analysis']['season_months']}; aggregation: {CONFIG['trend_analysis']['annual_agg']}",
                'Sensitivity': "; ".join(sensitivity_notes) if sensitivity_notes else "Not performed"
            }
        )

        append_stats_to_existing_summary(output_manager, stats_map, sensitivity_notes)
        logger.info("Summary JSON, README, and updated summary.txt generated")
...
def main():
    ...
    try:
        logger.info(f"\n{'='*60}")
        logger.info("Processing HAIG GLACIER - Trend Analysis")
        logger.info(f"{'='*60}")

        modis, aws, temp = loader.load_haig_data_complete()
        modis_sel = selector.select_best_pixels(modis, 'haig')
        ts = processor.prepare_time_series_data(modis_sel, aws, temp, 'haig')
        ...
        if CONFIG.get('altitude_paper', {}).get('run', False):
            ...
            append_table2_like_summary(output_manager, elev_med, edge_res, pure_res, month, min_gf)
        ...
        plt.show()
        generate_trend_summary_and_readme(output_manager, results, all_ts, sensitivity_notes)
