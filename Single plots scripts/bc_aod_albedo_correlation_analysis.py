...
CONFIG = {
    ...
    'output_config': {
        'base_dir': "outputs",
        'prefix': "bc_aod_albedo_correlation"
    },
    'visualization': {
        'figsize': (15, 10),
        'dpi': 300,
...
class BCAlbedoCorrelationAnalyzer:
    ...
    def load_and_process_data(self) -> pd.DataFrame:
        ...
        # Filter MODIS data to match MERRA2 temporal range
        years = self.config['temporal_filters']['years']
        months = self.config['temporal_filters']['months']
        logger.info(f"Filtering MODIS data to months {months} for years {years}...")
        modis_filtered = modis_df[
            (modis_df['date'].dt.year.isin(years)) &
...
def main():
    ...
    logger.info("Starting BC AOD vs Albedo Correlation Analysis")
    
    # Initialize and run analysis using CONFIG
    analyzer = BCAlbedoCorrelationAnalyzer(CONFIG)
