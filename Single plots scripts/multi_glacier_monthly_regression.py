#!/usr/bin/env python3
"""
Multi-Glacier Monthly Multiple Regression (configurable)
- Monthly aggregation:
...
logger = logging.getLogger(f"{CURRENT_CONFIG['output_prefix']}_monthly_regression")
...
CONFIG = {
    ...
    'output': {
        'analysis_name': f'{CURRENT_CONFIG["output_prefix"]}_monthly_regression',
        'base_dir': 'outputs',
        'summary_template': {}
    },
...
class DataLoader:
    ...
    def load_glacier_data_complete(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"Loaded {len(modis):,} MODIS, {len(aws):,} AWS albedo, {len(temp):,} temperature, and {len(precip):,} precipitation records for {CURRENT_CONFIG['name']}")
...
def plot_pairs_and_save(df: pd.DataFrame, run_dir: Path):
    ...
    colors = ['#0072B2', '#D55E00', '#009E73', '#CC79A7']  # Professional color palette
    
    for ax, (xcol, ycol, xlabel, ylabel), color in zip(axes, pairs, colors):
        if xcol not in df.columns or ycol not in df.columns:
            ax.axis('off'); continue
        x = df[xcol].values; y = df[ycol].values
        ax.scatter(x, y, color=color, s=35, alpha=0.8, edgecolor='none', zorder=3)
        if len(x) > 1 and np.ptp(x) > 0:
            slope, intercept, r, p, _ = stats.linregress(x, y)
            xx = np.linspace(np.nanmin(x), np.nanmax(x), 100)
            yy = slope * xx + intercept
            ax.plot(xx, yy, color='#333333', linewidth=1.8, zorder=2)
            
            # Add consolidated statistics box
        ...
        ax.set_xlabel(xlabel, fontsize=11); ax.set_ylabel(ylabel, fontsize=11); ax.grid(True, alpha=0.3)
        if 'Albedo' in ycol:
            ax.set_ylim(0, 1)
...
def partial_regression_plot_multi(df: pd.DataFrame, y_col: str, x_col: str, other_cols: List[str], out_path: Path):
    ...
    fig.suptitle(f'Partial Regression Analysis: {CURRENT_CONFIG["name"]} - {y_col} ~ {x_col}', 
                fontsize=13, fontweight='bold', y=0.95)
    
    # Use professional colors
    color_map = {'Temperature': '#D55E00', 'BC_AOD': '#009E73', 'Precipitation': '#0072B2'}
    scatter_color = color_map.get(x_col, '#7D3C98')
    
    ax.scatter(x_resid, y_resid, color=scatter_color, alpha=0.8, s=40, edgecolor='none', zorder=3)
    if len(x_resid) > 1 and np.ptp(x_resid) > 0:
        ...
        # Add consolidated statistics box
        sig_stars = "**" if p < 0.01 else "*" if p < 0.05 else ""
        stats_text = f'R={r:.3f}{sig_stars}  p={p:.3f}  n={len(x_resid)}\nPartial: R²={r**2:.3f}\nSlope={slope:.4f}'
        ax.text(0.02, 0.98, stats_text, transform=ax.transAxes,
                va='top', ha='left', fontsize=8, 
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9, linewidth=0.5))
    
    ax.set_xlabel(f"{x_col} residuals | {others_label}", fontsize=11)
    ax.set_ylabel(f"{y_col} residuals | {others_label}", fontsize=11)
...
def residual_diagnostics_plots(y: np.ndarray, y_hat: np.ndarray, run_dir: Path):
    ...
    # Residuals vs fitted
    fig, ax = plt.subplots(figsize=(9, 6), dpi=300)
    fig.suptitle(f'Residuals vs Fitted: {CURRENT_CONFIG["name"]} Monthly Regression Diagnostics', 
                fontsize=13, fontweight='bold', y=0.95)
    
    ax.scatter(y_hat, resid, color='#0072B2', alpha=0.8, s=35, edgecolor='none', zorder=3)
    ax.axhline(0, color='#D55E00', linewidth=1.8, zorder=2)
    ax.set_xlabel("Fitted values", fontsize=11); ax.set_ylabel("Residuals", fontsize=11)
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout(rect=[0, 0.04, 1, 0.92])
    fig.savefig(run_dir / RESIDUALS_PLOT, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
    plt.close(fig)
    
    # Q-Q Plot
    fig, ax = plt.subplots(figsize=(9, 6), dpi=300)
    fig.suptitle(f'Residuals Q-Q Plot: {CURRENT_CONFIG["name"]} Monthly Regression Diagnostics', 
                fontsize=13, fontweight='bold', y=0.95)
    
    stats.probplot(resid, dist="norm", plot=ax)
    # Update the Q-Q plot styling
    ...
    plt.tight_layout(rect=[0, 0.04, 1, 0.92])
    fig.savefig(run_dir / QQ_PLOT, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
    plt.close(fig)
