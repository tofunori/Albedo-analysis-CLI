...
class StatisticalAnalyzer:
    ...
    def calculate_basic_metrics(self, observed: pd.Series, predicted: pd.Series) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'mae': mae,
            'bias': bias,
            'relative_bias': np.mean((pred_clean - obs_clean) / obs_clean) * 100 if len(obs_clean) > 0 else np.nan,
            'r2': r_squared,
            'correlation': r_value,
    ...
    def run_comprehensive_analysis(self, data: pd.DataFrame) -> Dict[str, Any]:
        """
        Run comprehensive statistical analysis on comparative glacier data.
        
        Args:
            data: DataFrame with comparative analysis results containing columns like:
                  glacier_id, method, r, rmse, bias, mae, n_samples
        
        Returns:
            Dictionary containing statistical test results and summaries
        """
        ...
        try:
            ...
            results['summary_statistics'] = {
                'total_glaciers': data['glacier_id'].nunique(),
                'total_methods': data['method'].nunique(),
                'total_observations': data['n_samples'].sum(),
                'correlation_range': {
                    'min': data['r'].min(),
                    'max': data['r'].max(),
                    'mean': data['r'].mean(),
            ...
            results['method_comparisons'] = method_stats.to_dict()
            
            # Glacier-wise comparisons
            glacier_stats = data.groupby('glacier_id').agg({
                'r': ['mean', 'std', 'count'],
                'rmse': ['mean', 'std'],
                'bias': ['mean', 'std'],
                'n_samples': 'sum'
            ...
            results['glacier_comparisons'] = glacier_stats.to_dict()
            
            # Find best performing combinations
            best_correlation = data.loc[data['r'].idxmax()]
            best_rmse = data.loc[data['rmse'].idxmin()]
            
            results['best_performers'] = {
                'best_correlation': {
                    'method': best_correlation['method'],
                    'glacier': best_correlation['glacier_id'],
                    'r_value': best_correlation['r'],
                    'rmse': best_correlation['rmse']
                },
                'best_rmse': {
                    'method': best_rmse['method'],
                    'glacier': best_rmse['glacier_id'],
                    'r_value': best_rmse['r'],
                    'rmse': best_rmse['rmse']
                }
            }
            
            # Significance tests (if sufficient data)
            if len(data) > 3:
                try:
                    # ANOVA test for methods
                    method_groups = [group['r'].values for name, group in data.groupby('method')]
                    if len(method_groups) > 1 and all(len(group) > 0 for group in method_groups):
                        from scipy.stats import f_oneway
                        f_stat, p_value = f_oneway(*method_groups)
                        results['significance_tests']['method_anova'] = {
                            'f_statistic': f_stat,
                            'p_value': p_value,
                            'significant': p_value < 0.05
                        }
                except Exception as e:
                    logger.warning(f"Could not perform ANOVA test: {e}")
